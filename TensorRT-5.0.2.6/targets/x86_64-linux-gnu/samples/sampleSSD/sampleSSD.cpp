...
void initEngine(IExecutionContext& context,int batchSize){
    const ICudaEngine& engine = context.getEngine();
    inputIndex = engine.getBindingIndex(kINPUT_BLOB_NAME);
...
void doInference(IExecutionContext& context, float* inputData, float* detectionOut, int* keepCount, int batchSize,int callNum)
{
    /*
    const ICudaEngine& engine = context.getEngine();
...
int main(int argc, char** argv)
{
    ...
    std::vector<std::string> imageList = {"bus1.ppm","bus2.ppm","bus3.ppm","bus4.ppm"}; //,"bus.ppm","bus.ppm","bus.ppm"}; // Input image list
    ...
    // Run inference
    int numBatchCalls = 2;
    initEngine(*context,N);
    for(int k=0;k<numBatchCalls;k++){
        for (int i = 0; i < N; ++i){
            if(k==0)
                imageList[i] = "bus.ppm";
            else
        ...
        doInference(*context, data, detectionOut, keepCount, N,k); //doInference(*context, data, detectionOut, keepCount, N,1);
        for (int p = 0; p < N; ++p)
        {
